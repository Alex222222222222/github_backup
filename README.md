...
- `SSH_REPOSITORIES`: optional JSON array of repository paths. When set, SFTP discovery is skipped. Relative paths are resolved under `SSH_ROOT_DIR`; paths beginning with `/` are used as absolute paths.
...
For a Git-over-SSH server without SFTP, set an explicit list, for example `SSH_REPOSITORIES='["one.git","/srv/other/two.git"]'`. The final path component is used as the backup name; duplicate names are rejected to prevent archive and S3 collisions. Explicit paths that are not Git repositories are treated as failures.
